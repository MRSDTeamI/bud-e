...
ini_condition= [0.1 0.1 0.1 0 0 0 1]';
...
%    err_ori = quatmultiply([des_q_r, des_q_i'], [tmp_q_r, inv_tmp_q_i']);
   ...
   % calculate jacobian
   tmp_jacobian = Jacobian_gen_jw_3dof(theta);
   
   % calculate the pose of next frame
   inv_jac = inv(tmp_jacobian*tmp_jacobian')
   d_theta = tmp_jacobian'*pinv(tmp_jacobian*tmp_jacobian') * [err_pos; err_ori(2:4)'];
