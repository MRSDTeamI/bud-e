...
// Pass to "plane_seg" topic for debugging
	...
	if (cluster_indices.size() > 0) {
		// Print out center coordinate of the first cluster we have
		//std::cerr << "------------------------------------------------------------" << std::endl;
		//for (int i = 0; i < cluster_indices.size(); i++) {
		//	Eigen::Vector4f centroid;
		//	//pcl::compute3DCentroid(*outPCseg, cluster_indices[0], centroid);
		//	pcl::compute3DCentroid(*outPCseg, cluster_indices[i], centroid);
		...
		//pcl::toPCLPointCloud2(*clustered_cloud, *object);  // PointXYZ to PointCloud2
    	//pcl::toPCLPointCloud2(*outPCseg, *object);  // PointXYZ to PointCloud2
    	pcl_conversions::fromPCL(*object, output); // PointCloud2 to sensor-msgs
    	output.header.frame_id = input->header.frame_id;
    	pub_temp.publish(output);
	...
	/*
	// print cluster point cloud extract.setInputCloud (outPCseg);
...
/*
	...
	seg_cyl.setDistanceThreshold (0.01);
	seg_cyl.setRadiusLimits (0.03, 0.06);
